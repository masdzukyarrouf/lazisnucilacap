        ...
        <tbody>
            @foreach ($konfirmasis as $konfirmasi)
                <tr class="border-t" wire:key="konfirmasi-{{ $konfirmasi->id_konfirmasi }}">
                    ...
                    <td class="px-4 py-2 text-center break-words">{{ $konfirmasi->campaign }}</td>
                    <td class="px-4 py-2 text-center">
                        <img src="{{ asset('storage/' . $konfirmasi->bukti) }}" alt="Bukti Transfer" class="block w-24 h-auto mx-auto">
                    </td>
                    <td class="px-4 py-2 text-center">
